...
其中，3张固定样本已完成真实测试：直接生成3次成功；旧模型分析后生成2次成功；升级 `qwen3.7-plus` 后，同一3张样本均使用严格 JSON Schema 一次性分析成功，并分别完成万相与 Qwen Image 生图。两种新版首次生成链路技术成功率均为3/3；Qwen Image 在两张带商家型号的样本中0/2复现文字，优于万相的2/2，但默认分辨率较低且部分改款仍偏保守。结果迭代已完成两轮完整链路和一次聚焦真实测试，所有模型请求均一次成功且没有复现水印；但只要把任一 Qwen 生成图再次用于编辑，就会快速产生锐化、颗粒和色彩噪点，单图链式、双图锚定和稳定首版重基均未通过。当前代码已改为每轮重新上传最初商品图，用选中方向和全部累计修改一次性重建结果，尚待真实复测；加上上线基础设施未补齐，当前不适合公开测试。
