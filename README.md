...
- 领域规则、Provider 和服务端路由共有23项自动化测试。
- 火山方舟 Seedream 调用脚本已完成，账号仍需开通目标模型。

其中，3张固定样本已完成真实测试：直接生成3次成功；旧模型分析后生成2次成功；升级 `qwen3.7-plus` 后，同一3张样本均使用严格 JSON Schema 一次性分析成功，并分别完成万相与 Qwen Image 生图。两种新版首次生成链路技术成功率均为3/3；Qwen Image 在两张带商家型号的样本中0/2复现文字，优于万相的2/2，但默认分辨率较低且部分改款仍偏保守。结果迭代测试发现，只要把任一 Qwen 生成图再次用于编辑，就会快速产生锐化、颗粒和色彩噪点，单图链式、双图锚定和稳定首版重基均未通过。当前代码已改为每轮重新上传最初商品图，用选中方向和全部累计修改一次性重建；白色绗缝样本的聚焦调用和390×844真实 H5 refinement 路由均通过视觉与技术验收，父子历史和下载也正常。该结论仍只有一个固定样本，加上上线基础设施未补齐，当前不适合公开测试。
