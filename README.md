...
其中，3张固定样本已完成真实测试：直接生成3次成功；旧模型分析后生成2次成功；升级 `qwen3.7-plus` 后，同一3张样本均使用严格 JSON Schema 一次性分析成功，并分别完成万相与 Qwen Image 生图。两种新版首次生成链路技术成功率均为3/3；Qwen Image 在两张带商家型号的样本中0/2复现文字，优于万相的2/2，但默认分辨率较低且部分改款仍偏保守。结果迭代也已用 Qwen Image 完成“首次生成、同方向再生成、连续修改2次、下载”的真实 API 测试，5个模型请求全部一次成功；但第二次修改出现明显色彩噪点，说明技术链路已通、视觉收敛尚未验收。代码已增加稳定基准图和上一版双图约束，仍需再次真实验证；加上上线基础设施尚未补齐，当前不适合公开测试。
