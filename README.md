...
- 分析后的高置信度可见事实会作为可选保留项，由用户确认后才成为生图硬约束；方向卡按移动端层级展开。结果页默认突出“改一下 / 再出一版 / 保存图片”，进入修改后可选择单一调整维度、输入自然语言或勾选常见问题；这些输入确定性合并，不额外调用理解模型，只有点击生成下一版才占1次生图额度。专业用户仍可进入详细人工核对。
