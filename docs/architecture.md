...
微信云端的第一条真实验收切片是无模型费用的基础设施探针：白名单按 OpenID 派生的不可逆用户指纹查询，临时图片按用户指纹隔离上传，`garment-api` 将探针记录持久化并允许所属用户重新读取和主动清理。该切片已在真实独立云环境完成上传、跨请求恢复和云文件/数据库记录双重删除验收。其后新增受环境开关保护的 Fake Provider 业务切片：分析返回明确标为 `testing-fake` 且16项事实全部为 unknown 的结构，生图把原图转存为测试结果；该切片已经在真实环境完成分析、首次生成、再次生成、父子任务与结果转存验证，不验证模型效果。继续修改、任务处理中退出后恢复和相册保存仍待真机交互验收。基础设施部署步骤与验证记录见 [微信云开发无模型费用验证](wechat-cloud-setup.md)。

跨运行时业务层已经新增 `packages/application`。当前切片定义了分析、生成任务、幂等、试用额度和资产仓储端口，并用可回滚的内存事务 Fake 验证“用户/全局额度原子预占 → 排队任务 → 幂等绑定”：并发重试只创建一个任务且只计一次额度，同键换请求会冲突，后续写入失败会回滚。Fastify 的首次生成和继续修改入口已经使用该准入用例，但仍搭配进程内仓储以保持单机 H5 行为。微信云函数代码已经实现 `garment_analyses`、`garment_assets`、`generation_jobs`、`idempotency_records`、`trial_usage` 的数据库适配器以及受控云文件上传、下载和删除适配器，并已由 Fake Provider 业务动作实际调用；五张集合已经在真实环境创建且只允许管理员访问，Fake 分析、任务、幂等、额度和资产记录已完成真实写入与查询。微信生成任务另增加事务执行租约：模型调用前允许过期租约由同一请求接管，模型调用开始后发生中断则保守失败，避免自动重复付费调用。每日清理触发器先检查有效共享引用，再删云文件和过期资产元数据；失败记录保留重试，并清理已过期的分析、任务与幂等记录。租约已完成自动化和持久适配器契约验证，清理也已真实删除1个专用过期文件和对应元数据。真实模型执行与真实中断恢复仍待验证，因此不代表云端付费业务任务已经可用。
...
当前已实现生图任务的核心四态。H5/Fastify 分析仍为同步请求，任务状态只保存在当前进程；微信 Fake Provider 路径会把分析、任务、幂等和额度写入持久仓储，并由客户端保存未完成 jobId 后恢复查询。上传前状态、取消、用户主动删除、跨进程 worker 租约与真实 Provider 失败恢复仍属于目标能力。
...
- 同一幂等键不能重复创建付费生图请求：Fastify 在当前进程内实现，微信 Fake 路径通过持久仓储实现并完成自动化验证。
...
当前已完成分析、资产、任务、幂等和试用额度的微信云数据库适配器及云文件存储适配器代码，Fake Provider 业务动作已经接入；真实环境已创建集合并完成分析、首次/再次生成、手工清理和定时过期清理切片。执行租约已经实现并部署，但真实中断场景尚未验收。全部真机交互、匿名会话和项目历史仍未完成。
