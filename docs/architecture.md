...
跨运行时业务层已经新增 `packages/application`。当前第一切片定义了分析、生成任务、幂等、试用额度和资产仓储端口，并用可回滚的内存事务 Fake 验证“用户/全局额度原子预占 → 排队任务 → 幂等绑定”：并发重试只创建一个任务且只计一次额度，同键换请求会冲突，后续写入失败会回滚。Fastify 的首次生成和继续修改入口已经使用该准入用例，但仍搭配进程内仓储以保持单机 H5 行为。微信云函数代码已经实现 `garment_analyses`、`garment_assets`、`generation_jobs`、`idempotency_records`、`trial_usage` 的数据库适配器以及受控云文件上传、下载和删除适配器，并已由 Fake Provider 业务动作实际调用；五张集合已经在真实环境创建且只允许管理员访问，Fake 分析、任务、幂等、额度和资产记录已完成真实写入与查询。真实模型执行、任务租约和自动清理尚未接入，因此不代表云端付费业务任务已经可用。
...
当前已完成分析、资产、任务、幂等和试用额度的微信云数据库适配器及云文件存储适配器代码，Fake Provider 业务动作已经接入；真实环境已创建集合并完成分析、首次/再次生成和清理切片，但未完成全部真机交互。匿名会话、项目历史、执行租约和定时清理仍未实现。
