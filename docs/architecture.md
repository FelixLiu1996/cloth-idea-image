...
跨运行时业务层已经新增 `packages/application`。当前第一切片定义了分析、生成任务、幂等、试用额度和资产仓储端口，并用可回滚的内存事务 Fake 验证“用户/全局额度原子预占 → 排队任务 → 幂等绑定”：并发重试只创建一个任务且只计一次额度，同键换请求会冲突，后续写入失败会回滚。Fastify 的首次生成和继续修改入口已经使用该准入用例，但仍搭配进程内仓储以保持单机 H5 行为。微信云函数代码已经实现 `generation_jobs`、`idempotency_records`、`trial_usage` 的事务适配器，Fake 数据库契约测试证明其可跨适配器实例恢复并原子扣减额度；真实集合、云端业务动作和模型执行尚未接入，因此不代表云端业务任务已经可用。
...
当前仅完成任务、幂等和试用额度的微信云数据库适配器代码与 Fake 契约测试，尚未在真实环境创建集合或接入业务动作；分析、资产、匿名会话和任务历史仍未实现。
