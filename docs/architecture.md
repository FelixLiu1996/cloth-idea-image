...
跨运行时业务层已经新增 `packages/application`。当前第一切片定义了分析、生成任务、幂等、试用额度和资产仓储端口，并用可回滚的内存事务 Fake 验证“用户/全局额度原子预占 → 排队任务 → 幂等绑定”：并发重试只创建一个任务且只计一次额度，同键换请求会冲突，后续写入失败会回滚。Fastify 现有路由和微信云数据库适配器尚未切换到这组端口，因此这只是迁移基础，不代表云端业务任务已经持久化。
