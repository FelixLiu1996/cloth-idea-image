...
- 根据当前锁定项、选中方向和异常类型生成本次页面内的人工核对计划；普通用户只做“满意/需要修改”快速反馈，需要修改时将所选问题确定性编译为继续修改指令，详细逐项状态作为可选入口。状态尚未持久化，也尚未调用独立视觉核对 Provider。
- 通过 Taro 平台适配层下载结果图。
