...
微信云端当前只实现无模型费用的基础设施探针：白名单按 OpenID 派生的不可逆用户指纹查询，临时图片按用户指纹隔离上传，`garment-api` 将探针记录持久化并允许所属用户重新读取和主动清理。该切片已在真实独立云环境完成上传、跨请求恢复和云文件/数据库记录双重删除验收，但不包含分析、生图或正式任务迁移。部署步骤见 [微信云开发无模型费用验证](wechat-cloud-setup.md)。

跨运行时业务层已经新增 `packages/application`。当前第一切片定义了分析、生成任务、幂等、试用额度和资产仓储端口，并用可回滚的内存事务 Fake 验证“用户/全局额度原子预占 → 排队任务 → 幂等绑定”：并发重试只创建一个任务且只计一次额度，同键换请求会冲突，后续写入失败会回滚。Fastify 的首次生成和继续修改入口已经使用该准入用例，但仍搭配进程内仓储以保持单机 H5 行为。微信云函数代码已经实现 `garment_analyses`、`garment_assets`、`generation_jobs`、`idempotency_records`、`trial_usage` 的数据库适配器以及受控云文件上传、下载和删除适配器；Fake 契约测试证明其可按用户隔离、跨适配器实例恢复、处理过期记录并原子扣减额度。真实集合、云端业务动作和模型执行尚未接入，因此不代表云端业务任务已经可用。
...
当前已完成分析、资产、任务、幂等和试用额度的微信云数据库适配器及云文件存储适配器代码，尚未在真实环境创建集合或接入业务动作；匿名会话、项目历史和定时清理仍未实现。
