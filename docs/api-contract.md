...
返回支持的业务模式、改款幅度、图片类型、上传上限、默认输出数量、分析能力、结构版本、异步生图任务和结果迭代能力，可用于后续动态表单。单机受控试用还返回：

- `trialAccessRequired`：是否要求用户输入试用访问码。
- `trialDailyAnalysisLimit`：当前进程每日分析上限，`0` 表示不限。
- `trialDailyGenerationLimit`：当前进程每日生图上限，`0` 表示不限。
- `assetRetentionHours`：本地结果保留小时数，`0` 表示不自动删除。

## 受控试用访问

配置 `TRIAL_ACCESS_CODE` 后，以下付费模型入口必须发送请求头 `X-Trial-Access-Code`：

- `POST /api/v1/analyses`
- `POST /api/v1/generations`
- `POST /api/v1/generations/:jobId/refinements`

访问码错误返回 `401 AUTH_TRIAL_ACCESS_DENIED`。任务查询和包含 UUID 的结果资源地址当前不要求访问码，访问码因此只是小范围费用保护，不是正式登录鉴权。

## 分析原款并创建设计方向

`POST /api/v1/analyses`

请求为 `multipart/form-data`，字段与下方创建改款的基础字段相同。客户端应发送独立的 `Idempotency-Key`；同一服务进程内重复提交相同键时不再次调用分析模型，也不重复占用当日分析额度。
...
客户端应发送唯一的 `Idempotency-Key` 请求头。同一服务进程内再次提交相同键时返回同一任务的当前状态，不重复调用模型或占用生图额度；同一键用于不同请求时返回冲突。
...
- 结果图保存在本地 `var/assets/`；可配置按结果目录修改时间自动清理，默认关闭。
- 单机试用只有共享访问码、全局进程内每日额度、并发和启动间隔，没有正式登录、匿名会话、用户/IP独立限流或持久预算。
- 每日额度按 UTC 日期计算，任务和计数在服务重启后丢失。
